...
const handleSignIn = async (e) => {
  ...
  // Validate server URL
  if (!serverUrl || typeof serverUrl !== "string" || !/^https?:\/\//.test(serverUrl)) {
    setErr("Server URL is not configured properly.");
    setLoading(false);
    return;
  }
  ...
  try {
    ...
    setErr(error?.response?.data?.message || "Failed to connect to server.");
    setUserData(null);
    setLoading(false);
  }
};
...
<h1 className='text-white text-[30px] font-seminbold mb-[30px]'>Sign In to <span className='text-blue-400'>Virtual Assistant</span></h1>
<input type='text' placeholder='Email'className='w-full h-[60px] outline- none border-2 border-white bg-transparent text-white placeholder-grey-300 px-[20px] py-[10px] rounded-full text-[18px]'required onChange={(e)=>setEmail(e.target.value)} value={email}/>
<div className='w-full h-[60px] border-2 border-white bg-transparent text-white  rounded-full text-[18px] relative'>
<input type={showPassword?"text":"password"} placeholder='password' className='w-full h-full rounded-full outline-none bg-transparent text-white placeholder-grey-300 px-[20px] py-[10px]'required onChange={(e)=>setPassword(e.target.value)} value={password}/>
